...
async function injectBugTracker() {
  ...
  const rows = [
    ...
    { id: rowIds[4], strict: false, cells: [
      cell(C.bugId, 'BUG-005'),
      cell(C.title, 'Missing logo asset breaks BMCTool build'),
      cell(C.module, 'Portal / BMC Tool'),
      cell(C.severity, 'Critical'),
      cell(C.priority, 'Critical'),
      cell(C.status, 'Resolved'),
      cell(C.reportedBy, 'Keenan'),
      cell(C.dateReported, '2026-02-20'),
      cell(C.dateResolved, '2026-02-20'),
      cell(C.environmentFound, 'development (Netlify + local dev)'),
      cell(C.branchFound, 'development'),
      cell(C.rootCauseCategory, 'Missing Asset'),
      cell(C.rootCauseDesc, 'BMCTool.tsx integrated from EDMECA-BMC-main had its logo at attached_assets/EdMeca_LOGOMARK_A4_2024_1767948366417.jpg. That file was not copied to the Edmeca-Website project. Vite @assets alias resolves to attached_assets/ which did not contain the file, causing build failure.'),
      cell(C.stepsToReproduce, '1. Checkout development branch. 2. Run npm run dev or npm run build. 3. Observe Vite import-analysis error on BMCTool.tsx line 3.'),
      cell(C.expectedBehaviour, 'App builds successfully and logo renders in BMC tool welcome screen'),
      cell(C.actualBehaviour, '[plugin:vite:import-analysis] Failed to resolve import @assets/EdMeca_LOGOMARK_A4_2024_1767948366417.jpg. Build fails.'),
      cell(C.fixSummary, 'Replaced missing @assets import with const edmecaLogo = \'/logo.png\' — uses the existing logo.png already present in client/public/ which is served at root by Vite.'),
      cell(C.linkedTask, '3.15'),
      cell(C.linkedTest, ''),
      cell(C.verifiedBy, 'Keenan'),
      cell(C.verificationDate, '2026-02-20'),
      cell(C.regressionRisk, 'Low'),
      cell(C.notes, 'logo.png in client/public/ is the canonical project logo used across email templates and site. No asset copy needed.'),
    ]},
    { id: rowIds[5], strict: false, cells: [
      cell(C.bugId, 'BUG-006'),
      cell(C.title, 'BMC Finalize fails — version column missing from artifacts'),
      cell(C.module, 'Portal / BMC Tool'),
      cell(C.severity, 'Critical'),
      cell(C.priority, 'Critical'),
      cell(C.status, 'Resolved'),
      ...
      cell(C.environmentFound, 'staging--edmecaacademy.netlify.app'),
      cell(C.branchFound, 'staging'),
      cell(C.rootCauseCategory, 'Schema Mismatch'),
      cell(C.rootCauseDesc, 'shared/schema.ts defines version INTEGER DEFAULT 1 on the artifacts table but the initial SQL migration (20260216085856_initial_schema.sql) omitted the column. Supabase PostgREST schema cache does not know about the column.'),
      cell(C.stepsToReproduce, '1. Log in on staging. 2. Complete BMC tool canvas. 3. Click Finalize & Save. 4. Observe error: Could not find the version column of artifacts in the schema cache.'),
      cell(C.expectedBehaviour, 'BMC canvas saves successfully to Supabase artifacts table'),
      cell(C.actualBehaviour, 'Error: Could not find the \'version\' column of \'artifacts\' in the schema cache. Save fails.'),
      cell(C.fixSummary, '1. Removed version: 1 from the supabase.from(artifacts).insert() call in BMCTool.tsx so the save works immediately. 2. Created migration 20260220120000_add_version_to_artifacts.sql to add the column to the live DB.'),
      cell(C.linkedTask, '3.15'),
      cell(C.linkedTest, ''),
      cell(C.verifiedBy, 'Keenan'),
      cell(C.verificationDate, '2026-02-20'),
      cell(C.regressionRisk, 'Low'),
      cell(C.notes, 'Run migration 20260220120000_add_version_to_artifacts.sql in Supabase dashboard to fully sync schema. Short-term: version omitted from insert so default still applies once column is added.'),
    ]},
    { id: rowIds[6], strict: false, cells: [
      cell(C.bugId, 'BUG-007'),
      cell(C.title, 'Tool data lost on navigation without Finalize'),
      cell(C.module, 'Portal / Tools'),
      cell(C.severity, 'High'),
      cell(C.priority, 'High'),
      cell(C.status, 'Resolved'),
      cell(C.reportedBy, 'Keenan'),
      cell(C.dateReported, '2026-02-21'),
      cell(C.dateResolved, '2026-02-21'),
      cell(C.environmentFound, 'local dev + staging'),
      cell(C.branchFound, 'development'),
      cell(C.rootCauseCategory, 'Missing Auto-Save'),
      cell(C.rootCauseDesc, 'Tool components (SWOTPestleTool, ValuePropTool, PitchBuilderTool) held all in-progress form state in React useState. No save to Supabase until Finalize was clicked. Navigating away unmounted the component and destroyed all unsaved state.'),
      cell(C.stepsToReproduce, '1. Log in and navigate to /portal/tools/swot. 2. Add several strengths, weaknesses, and PESTLE items. 3. Click ← Dashboard without clicking Finalize. 4. Navigate back to /portal/tools/swot. 5. Observe all entered items are gone.'),
      cell(C.expectedBehaviour, 'Previously entered items persist when user returns to the tool'),
      cell(C.actualBehaviour, 'All unsaved data is lost on navigation — user must re-enter everything'),
      cell(C.fixSummary, 'Added debounced auto-save useEffect (1500ms) to SWOTPestleTool, ValuePropTool, PitchBuilderTool. Watches data state, calls artifactsService.saveArtifact() with status=in_progress silently. Uses hasLoadedRef to skip initial mount and existingIdRef to avoid stale closures. ProgressTrackerTool unaffected — each item saved immediately on add.'),
      cell(C.linkedTask, '3.4'),
      cell(C.linkedTest, ''),
      cell(C.verifiedBy, 'Keenan'),
      cell(C.verificationDate, '2026-02-21'),
      cell(C.regressionRisk, 'Low'),
      cell(C.notes, 'Auto-save fires silently — no toast. Manual Save Draft button still available. ProgressTrackerTool not affected.'),
    ]},
  ];
