...
async function injectBugTracker() {
  ...
  const rows = [
    ...
    { id: rowIds[6], strict: false, cells: [
      ...
      cell(C.rootCauseDesc, 'Tool components (SWOTPestleTool, ValuePropTool, PitchBuilderTool, ProgressTrackerTool) hold all in-progress form state in React useState. No save to Supabase until Finalize is clicked. Navigating away unmounts the component and destroys all unsaved state.'),
      ...
      cell(C.fixSummary, 'Add debounced useEffect auto-save in each tool — calls artifactsService.saveArtifact() with status=in_progress on every data change. Load-on-mount already works (useQuery for getLatestArtifactByType pre-fills state on re-entry).'),
      cell(C.linkedTask, '3.4'),
      cell(C.linkedTest, ''),
      cell(C.verifiedBy, ''),
      cell(C.verificationDate, ''),
      cell(C.regressionRisk, 'Low'),
      cell(C.notes, 'Affects all 4 portal tools. Fix is purely adding debounced auto-save before finalize is clicked. The round-trip restore on mount is already implemented.'),
    ]},
  ];
